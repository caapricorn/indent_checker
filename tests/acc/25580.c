...
node_t * create_node(int k, char * v) {
    ...
    x->spans = (int *)calloc(MAX_LEVELS, sizeof(int));
    return x;
}
...
node_t * create_list(void) {
    ...
    n->spans = (int *)calloc(MAX_LEVELS, sizeof(int));
...
void insert(node_t * l, int k, char * v) {
    ...
    for (; i < MAX_LEVELS && r % 2 == 0; ++i, r /= 2) {
        ...
        x->spans[i] = p[i]->spans[i] + 1 - (rank_x - temp_rank);
        p[i]->spans[i] = rank_x - temp_rank;
    }
