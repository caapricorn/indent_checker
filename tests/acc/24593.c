...
{
    long x;
    // long *queue1 = (long *)malloc(4 * sizeof(long));
	long *qs[2];
    ...
    while (1)
    {
        ...
        if (strcmp("ENQ", s) == 0)
        {
            ...
            else
            {
                ...
                // printf("%ld\n", qs[ind][head]);
                head++;
                // printf("element was addet sucsessfully\n");
            }
            if (head == len)
                head = 0;
            is_last_add = 1;
        ...
        // printf("head = %d, tail = %d, is_last_add = %d\n", head, tail, !is_last_add);
    }
    free(qs[ind]);
    return 0;
}